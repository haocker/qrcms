...
class WeixinAction extends CommonAction{
    public function index() {
        ...
        $list   = $Form->Table($prefix.'weixin w')->join('(select * from '.$prefix.'tuijian WHERE recommendid=3 AND (timelimit=0 OR endtime>=unix_timestamp(CURDATE())) AND status=1) t ON w.id=t.wxid')->where($map)->field('w.id,w.pubaccount,w.logo,w.weblogo,w.qrcode,w.webqrcode,w.content,w.tag,w.xingji,w.hits,w.xh,w.title,w.status,w.city,t.recommendid,IFNULL(t.create_time,9369311152) as tjtime')->limit($Page->firstRow. ',' . $Page->listRows)->order('tjtime ASC')->select();
